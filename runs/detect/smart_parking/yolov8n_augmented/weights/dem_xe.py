...
# ====================================================================
CONFIG = {
    'model_path': 'best.onnx',           # Đổi thành 'best.pt' nếu muốn test
    'video_source': 'video_demo.mp4', # Đổi thành 0 nếu chạy webcam
    'target_classes': [0, 1, 2, 3],      # Các ID class cần theo dõi
    'class_names': {0: 'Car', 1: 'Moto', 2: 'Bus', 3: 'Truck'},
    
    'conf_thres': 0.45,                  # Ngưỡng tin cậy (Lọc rác)
    'iou_thres': 0.5,                   # Ngưỡng chống đè khung (NMS)
    'tracker_type': 'bytetrack.yaml'   # Thuật toán Tracking
}
...
output_path = "outputs/result_videodemo_FP16.mp4"
...
# Đặt vạch đếm ở mức 70% màn hình từ trên xuống
Y_LINE = int(H * 0.7)
LINE_START, LINE_END = (0, Y_LINE), (W, Y_LINE)

# Vùng bỏ qua xử lý (Ignored Area) ở mức 30% màn hình trên cùng
Y_IGNORE = int(H * 0.3)
...
while cap.isOpened():
    ...
    # Vẽ ranh giới phân cách vùng bỏ qua (Tùy chọn: Vẽ nét đứt hoặc mờ để quan sát)
